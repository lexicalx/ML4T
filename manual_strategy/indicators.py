...
def rsi(prices, window=14, min_periods=0):
    ...
    rsi.rename(columns={'JPM': 'RSI'}, inplace=True)
    return rsi
...
def priceOverSMA(prices):
    ...
    priceOverSma.rename(columns={'JPM': 'Price/SMA'}, inplace=True)

    sma_JPM.rename(columns={'JPM': 'SMA'}, inplace=True)
    sma_JPM['JPM'] = prices['JPM']
    plot_fig(sma_JPM, 'JPM - SMA', (12, 6), 'Date', 'SMA', 'SMA-in-sample')
...
if __name__ == "__main__":
    ...
    plot_fig(rsi, 'JPM - RSI', (12, 6), 'Date', '', 'RSI-in-sample')
